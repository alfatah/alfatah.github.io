...
function getStockIndex(countryCode) {
    var apiKey = 'cpf338hr01qh4lv5hnmgcpf338hr01qh4lv5hnn0'; // Replace with your Finnhub API key
    var symbol;

    // Map country code to a stock index symbol (example with some indices, you may need to expand this)
    switch(countryCode) {
        case 'US':
    ...
    $.getJSON(url, function(data) {
        if (data && data.c !== undefined) {
            var stockInfo = `
                Stock Index (${symbol}):
                <br>Current Price: ${data.c}
                <br>High Price: ${data.h}
                <br>Low Price: ${data.l}
                <br>Previous Close: ${data.pc}
            `;
            $("#stock-index").html(stockInfo);
        } else {
            $("#stock-index").html('Stock data not available.');
        }
    });
}